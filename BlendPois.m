%% mblend merges a specific region in the first image to a user specified region in the second image using multi-band blending
function im = BlendPois(image_name1, image_name2,COL,ROW)
im1 = image_name1;
im2 = image_name2;
im1 = im2double(im1);
im2 = im2double(im2);
[mk, xi, yi] = roipoly(im1,COL,ROW);
xi = floor(xi);
yi = floor(yi);
im1 = imcrop(im1, [min(xi), min(yi), max(xi)-min(xi), max(yi)-min(yi)]);
mk = imcrop(mk, [min(xi), min(yi), max(xi)-min(xi), max(yi)-min(yi)]);
[mt, xi, yi] = roipoly(im2,COL,ROW);
xi = floor(xi);
yi = floor(yi);
